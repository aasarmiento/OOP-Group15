...
public class CSVHandler implements EmployeeDAO {
    ...
    // --- ADDED MAPPING METHODS FOR IT SERVICE ---

    /**
     * Maps the requested 'getAllEmployees' to your existing 'getAll' method.
     */
   public List<Employee> getAllEmployees() {
    return getAll(); // This assumes 'getAll()' is defined in your CSVHandler or Parent class
}

    /**
     * Maps the requested 'updateEmployeeFile' to your existing 'saveAllToCSV' method.
     */
    public void updateEmployeeFile(List<Employee> employees) {
        // Since your cache is the source of truth, we ensure the cache is updated 
    ...
    /**
     * Unlocks the account and saves the change.
     */
    @Override
    public void unlockAccount(int employeeNo) {
        Employee emp = findById(employeeNo);
        if (emp != null) {
            // Ensure your Employee model has setAccountStatus or use setStatus
...
@Override
  public void saveProfilePicture(int empNo, File sourceFile) throws IOException {
    // 1. Define where the images should live inside your project
    File storageDir = new File("resources/profile_pics/");
    if (!storageDir.exists()) storageDir.mkdirs();

    // 2. Create the destination file using the Employee Number
    // Result: resources/profile_pics/10002.png
    String extension = getFileExtension(sourceFile.getName());
    File destination = new File(storageDir, empNo + extension);

    // 3. Perform the actual streaming (The 'Data' part of DAO)
